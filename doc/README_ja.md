※ 翻訳内容と英語版の間で齟齬、不一致または矛盾がある場合、英語版が優先します。
...
* /var/log/secure ファイルのタイムスタンプを UNIX 時刻に変換

```bash
$ cat /var/log/secure | teip -c 1-15 -- date -f- +%s
```

* 「'HELLO' を含んでいる行」の 'WORLD' を 'EARTH' に置換

```bash
$ cat file | teip -g HELLO -- sed 's/WORLD/EARTH/'
```

* CSV ファイル ( RFC4180 に沿う ) の 2 列目のみを大文字にする

```bash
$ cat file.csv | teip --csv -f 2 -- tr a-z A-Z
```

* TSV ファイルの 2, 3, 4 列目を編集する

```bash
$ cat file.tsv | teip -D '\t' -f 2-4 -- tr a-z A-Z
```

* 'hello' を含んだ行と、その前後 3 行目のみを編集する
...
`teip` により、任意のコマンドが自身のタスクに集中できるようになります。

下記は、100 MiB のファイルに含まれる 761,000 の IP アドレスをダミーの文字列に置換する際に要した時間の比較です。

<p align="center">
  <img src="https://raw.githubusercontent.com/wiki/greymd/teip/benchmark/secure_bench.svg" width="80%" alt="benchmark bar chart" />
</p>

詳しくは <a href="https://github.com/greymd/teip/wiki/Benchmark">wiki > Benchmark</a>.

## 機能

* テーピング: コマンドが「ひとつのことを上手くやる」のを助ける
  - 標準入力の一部のみを任意の対象のコマンドにバイパスする
  - 対象のコマンドはバイパスされた部分の標準入力のみを処理
  - AWK や cut や grep のように、柔軟な方法でバイパス範囲を指定可能

* 高速処理
  - 対象のコマンドの標準入出力は `teip` のスレッドにより非同期に中継される
  - もしあなたの環境で、とあるコマンドが 数 100 MB のファイルを数秒で処理できるのならば、`teip` の対象となったそのコマンドは同等ないしそれ以上のパフォーマンスの発揮が期待できる
...
<!-- release_url_start -->
必要に応じ、[最新のリリースページ](https://github.com/greymd/teip/releases/tag/v2.2.0) のハッシュ値を確認してください。`sha256` で終わるファイル名にハッシュ値があります。
<!-- release_url_end -->


### macOS (x86_64, ARM64)

[Homebrew](https://brew.sh/) を使ってください。
...
<!-- ins_url_start -->
インストーラを[ここ](https://github.com/greymd/teip/releases/download/v2.2.0/teip_installer-2.2.0-x86_64-pc-windows-msvc.exe) からダウンロードしてください。
<!-- ins_url_end -->

詳しくは [Wiki > Use on Windows](https://github.com/greymd/teip/wiki/Use-on-Windows) を参照。

### その他のアーキテクチャ

<!-- release_url_start -->
[最新のリリースページ](https://github.com/greymd/teip/releases/tag/v2.2.0) を確認して、希望の環境の実行ファイルがあるか確認してください。
<!-- release_url_end -->

もしなければ、ソースからビルドしてください。

### ソースからのビルド

`cargo` コマンドであれば下記の方法です。

```
cargo install teip
```

鬼車正規表現（`-G` オプション）を有効にするためには、`--features oniguruma` オプションをツケます。
`libclang` の共有ライブラリが利用環境に事前に入っていることを確認してください。
...
結果は、入力とほぼ同じですが "300" の箇所がハイライトされ、 `[...]` に囲まれていると思います。
なぜなら、`-f 3` により、空白区切りの 3 列目が指定されたためです。

```bash
100 200 [300] 400
```

`[...]` で囲まれたエリアはマスキングテープの **穴** のようなものと理解をしてください。

<img src="https://raw.githubusercontent.com/wiki/greymd/teip/img/teip_hole.png" width="300" />

次に `sed` コマンドを `teip` コマンドの引数の末尾にそのまま追記してみましょう。
...
ご覧の通り、`sed` は **穴** の中にあった入力のみを処理し、他のマスクされた部分は無視しています。
技術的には `teip` はハイライトされた部分だけを `sed` に渡し、`sed` の出力した結果と元の入力を置換しています。

もちろん、他のコマンドも指定可能です。
このコマンドは **対象コマンド** とこの記事の中で呼びます。
